        ...
        if self.args.train:
            for epoch in tqdm(range(self.args.start_epoch, self.args.final_epoch + 1)):
                ...
                print('\nEpoch {}/{} : train loss {} | test loss {} | test top1 err {} | test top5 err {} | current lr {} | took {} h {} m {} s'.format(
                    epoch, self.args.final_epoch, train_results[0], test_results[0], test_results[1], test_results[2], self.current_lr(self.optimizer), int(h), int(m), int(s)))
                ...
                if self.args.save_cpt_interval is not None and epoch % self.args.save_cpt_interval == 0 or self.args.final_epoch == epoch:
                    save_result(self.args, self.model, self.optimizer, self.scheduler, self.history, test_results, epoch)
